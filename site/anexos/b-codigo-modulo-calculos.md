# Anexo B: Código del módulo de cálculos



*Listado del código fuente original de Explocal (1996). No se ha podido recuperar la indentación original del código.*
...
macros y estructuras de datos necesarias para calcular las características de un explosivo.
...
#define K_VCN 0.02388459 // [m³/mol]
#define PRECISION_BO 0.001
...
char FormulaBO[6]; /* Producto para el cálculo del balance de oxígeno:
...
devuelve 0 , si no sería casi imposible encontrar explosivo perfectamente equilibrados.
-------------------------------------------------------------------------*/
#define BO_PRODBO 1
#define BO_PRODEX 2
...
if(fabs(respuesta)<PRECISION_BO) respuesta=0.0;
}
return respuesta;
}
...
if (acumulador)
{
sprintf(Cadena,"%s%.6f\0",TablaProd[i].Simbolo,acumulador);
...
gaseosos.
Nota I: Si los moles de gas Ng[g/mol] son nulos, M lo debe ser también.
...
double nCO: valor.
Resultado: double: Valor de la función polimonica para nCO.
Ninguno.
Descripción:
--------------------------------------------------------------------------*/
...
CO2,CO,C,H2O y H2. Además verifica si la solución obtenida es admisible (valores positivos)
Nota I: La función solo funciona con mezclas con B.O. negativo.
-------------------------------------------------------------------------*/
...
for(Grafito=SIN_GRAFITO;Grafito<=CON_GRAFITO;Grafito++)
{
...
// Si hay solución válida sin grafito salir del bucle...
if (Es_solucion==SI) break;
}
