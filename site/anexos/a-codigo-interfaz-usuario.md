# Anexo A: Código del interfaz de usuario



*Listado del código fuente original de Explocal (1996), en Borland C++ 3.1 con la librería Object Windows (OWL). Se reproduce tal cual en el documento original; no se ha podido recuperar la indentación original del código, perdida en la conversión desde el formato binario de Word 6.0 de 1996.*

## A.1 CÓDIGO DEL MÓDULO PRINCIPAL (EXPLOCAL.CPP)
...
Descripción: Listado del programa principal que incorpora EL in-
...
virtual void CMEditSelT(RTMessage Msg)=[CM_FIRST+CM_EDITSELT];
virtual void CMGuardExp(RTMessage Msg)=[CM_FIRST+CM_GUARDEXP];
virtual void CMGComoExp(RTMessage Msg)=[CM_FIRST+CM_GCOMOEXP];
virtual void CMComposic(RTMessage Msg)=[CM_FIRST+CM_COMPOSIC];
virtual void CMFilePrint(RTMessage Msg)=[CM_FIRST+CM_IMPRIMIR];
};
/*__________________________________________________________________________
...
Nota I: No es necesario comprobar si está disponible el archivo de datos puesto que de faltar, 
    el programa no pasa la opción "Iniciar". */
...
{
SendDlgItemMsg(IDLstDestino,LB_RESETCONTENT,0,0L);
// Borra L. destino.
// Si hay por lo menos un elemento en la lista 2.
if(SendDlgItemMsg(IDLstDatos, LB_GETCOUNT, 0, 0L )!=0)
{
// Obtiene el índice del elemento de la lista 2 que tiene el // rectángulo foco,carga el elemen
    to y lo envía a la lista 3
...
sprintf(Cadena1,"%.3f g/mol",PMol); //Peso molecular con 3 decim
...
{
fgets(Cadena2,MAXCAD,f);
// Salta el campo de fórmulas.
fgets(Cadena2,MAXCAD,f);
// Salta el campo de energías de form.
// Elimina el caracter intro: de no hacerlo los nombres // de los compuestos acabarán con un cu
    adrado al salir en //la lista.
...
reactivo seleccionado, baja la barra de selección al reactivo inmediatamente inferior tanto en 
    la lista de porcentajes de cada
...
while (!feof(b))
// mientras no se llege al final del archivo...
{
...
de diálogo.
__________________________________________________________________________*/
// OK: Respuesta a la pulsación de la tecla aceptar del diálogo 5.
...
Cadena1[(WORD)SendDlgItemMsg(ID5_EDITA1,EM_GETLINE,0,
(DWORD)(LPSTR)Cadena1)]='\0';
sprintf(Cadena2,"%s\r\n\0",Cadena1);
fputs(Cadena2,f);
Cadena1[(WORD)SendDlgItemMsg(ID5_EDITA2,EM_GETLINE,0,
(DWORD)(LPSTR)Cadena1)]='\0';
sprintf(Cadena2,"%s\r\n\0",Cadena1);
fputs(Cadena2,f);
Cadena1[(WORD)SendDlgItemMsg(ID5_EDITA3,EM_GETLINE,0,
(DWORD)(LPSTR)Cadena1)]='\0';
sprintf(Cadena2,"%s\r\n\0",Cadena1);
fputs(Cadena2,f);
EndDialog(HWindow,IDOK);
...
/* BTINTR: Al Pulsar la tecla INTRO: El foco se mueve al siguiente elemento
del DIALOGO5, es decir consigue que INTRO realice la misma misión que el tabulador. Esto se tie
    ne en cuenta para facilitar la entrada de datos */
...
directorio[(WORD)SendDlgItemMsg(ID6_EDITA1,EM_GETLINE,0,
...
switch((Xpl.Resultado.BO==0.0)? IGUAL:(Xpl.Resultado.BO>0.0)? MAYOR:MENOR)
{
case MENOR:
...
PonerEnEditor(IDS_PRQEXP,Xpl.Resultado.Qexplosion,Unidades);
// Ajuste(Xpl.Resultado.Qexplosion,5.0*JUL_A_CAL),Unidades);
PonerEnEditor(IDS_PRTEXP,Ajuste(Xpl.Resultado.Texplosion,5.0));
...
PonerEnEditor(IDS_PRVELD,Ajuste(Xpl.Resultado.Dcj,10.0));
...
{
Xpl.CalcResultado();
// Calcular todas las carácterísticas del explosivo.
PonerResultados();
// Envía los resultados al editor de texto.
}
else
{
...
en el archivo de inicilización: EXPLOCAL.INI*/
...
/* CMABRIREXP: (Explosivo/Abrir) contruye una ventana hija con el resultado del problema cuyos 
    datos se especifican, mediante archivo*/
...
## A.2 CABECERA CON LAS MACROS (EXPLOCAL.H)
...
de EXPLOFAB.CPP así como la definición de todos
las macros.
_____________________________________________________________________*/
// Macro para la compilación condicional.
#define __EXPLOCAL_H
#if !defined(ID_INPUT)
#include "owlrc.h"
#endif
...
#define ID6_BRJULS 101 // Diálogo 6
...
NAME EXPLOCAL
DESCRIPTION 'Explocal: Enrique Pérez Herrero - 1996'
EXETYPE WINDOWS
STUB 'WINSTUB.EXE'
